    ...
    public partial class Main : Form
    {
        ...
        public ItemDetails GetItemDetails(string itemNumber, string warehouse, string qtyRequested)
        {
            string req = $"<?xml version='1.0' encoding='UTF-8'?><!DOCTYPE System-Link SYSTEM 'SystemLinkRequest.dtd'>"+
                $"<System-Link><Login userId='{Program.xaLogin.Key}' password='{Program.xaLogin.Value}' maxIdle='30'\r\n"+
                ...
                $"                 sessionAlias='ExcelToXA'/><Request sessionHandle='*current' workHandle='*new'\r\n" +
            ...
            JObject resp = SendWebRequest(req);
        ...
        public void UploadItem(string poNumber, ItemDetails id)
        {
            string req = $"<?xml version='1.0' encoding='UTF-8'?><!DOCTYPE System-Link SYSTEM 'SystemLinkRequest.dtd'><System-Link>" +
                $"<Login userId='{Program.xaLogin.Key}' password='{Program.xaLogin.Value}' maxIdle='30'\r\n" +
                $"          properties='com.pjx.cas.domain.EnvironmentId={userSettings.Environment.DisplayText.Substring(0, 2)},\r\n" +
                $"               com.pjx.cas.domain.SystemName=INFXAPRD,\r\n" +
                $"               com.pjx.cas.user.LanguageId=en'\r\n" +
                $"               sessionAlias='ExcelToXA'/><Request sessionHandle='*current' workHandle='*new'\r\n" +
                ...
                $"</System-Link>\r\n";
            JObject response = SendWebRequest(req);
        ...
        public void GetOrders()
        {
            string req = $"<?xml version='1.0' encoding='UTF-8'?><!DOCTYPE System-Link SYSTEM 'SystemLinkRequest.dtd'><System-Link>" +
                ...
                $"               com.pjx.cas.user.LanguageId=en'\r\n" +
                $"               sessionAlias='ExcelToXA'/><Request sessionHandle='*current' workHandle='*new'\r\n" +
                $"               broker='EJB' maxIdle='30'>" +
                $"<QueryList name='queryListPurchaseOrder_GeneralR7' domainClass='com.mapics.pm.PurchaseOrder' includeMetaData='true' maxReturned='100'>" +
                ...
                $"                          {DateTime.Now.Subtract(TimeSpan.FromDays(7)).ToString("yyyyMMdd")} ORDER BY order DESC]]></Pql></QueryList></Request></System-Link>\r\n";
            JObject resp = SendWebRequest(req);
            if (resp == null) return;
            orders.Clear();
        ...
        public JObject SendWebRequest(string xmlReq)
        {
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                bool login = bool.Parse(JsonConvert.DeserializeObject<string>(resp["System-Link"]["LoginResponse"]["@actionSucceeded"].ToString()));
                
                if (login)
                    return resp;
                else
                {
                    JArray msgs = (JArray)resp["System-Link"]["LoginResponse"]["Exception"]["Message"];
                    string errorMsg = "";
                    foreach (JObject obj in msgs)
                    {
                        switch (obj["@type"].ToString())
                        {
                            case "error":
                                errorMsg = obj["Text"]["#cdata-section"].ToString();
                                break;
                        }
                    }
            ...
            {
                ShowError("Malformed XML request was sent.", "Bad Request");
                return null;
            }
            return null;
        }
